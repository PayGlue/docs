...
<Note>Placeholder: Paywall overlay as it appears on a Ghost post (button, headline, body text visible)</Note>
...
<Steps>
  ...
  <Step title="Create a Paywall config">
    ...
    **Custom URL** — paste any URL manually. Use this for providers that don't yet support the product picker, for PWYW links, or for Ghost portal links.
    ...
    <Note>
      If you see **Sandbox mode — using test products**, your Polar token is set to the test environment. Sandbox purchases do not trigger production webhooks. Switch to a production token in **Connection > Polar** to go live.
    </Note>

    <Note>Placeholder: PayGlue Dashboard — Paywall editor with live preview on the right</Note>
